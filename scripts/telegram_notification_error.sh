...
send_telegram_alert() {
    ...
    # Validate inputs
    if [ -z "$TELEGRAM_ERROR_DETECTION_TOKEN" ]; then
        echo "❌ Telegram Bot Token is missing"
        exit 1
    fi

    if [ -z "$TELEGRAM_ERROR_DETECTION_CHAT_ID" ]; then
        echo "❌ Telegram Chat ID is missing"
        exit 1
    fi

    # Telegram API URL
    TELEGRAM_API_URL="https://api.telegram.org/bot$TELEGRAM_ERROR_DETECTION_TOKEN/sendMessage"

    # Send message
    RESPONSE=$(curl -s -X POST "$TELEGRAM_API_URL" \
        -d "chat_id=$TELEGRAM_ERROR_DETECTION_CHAT_ID" \
        -d "text=$MESSAGE")
