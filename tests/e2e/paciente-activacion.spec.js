...
test.describe("Flujo de Activación de Cuenta de Paciente", () => {
    ...
    test.beforeAll(async () => {
        ...
        execSync("php artisan db:seed --class=UserSeeder", {
            stdio: "inherit",
        });
    });
    ...
    test("debería crear un nuevo paciente y enviar correo de activación", async ({
        page,
    }) => {
        // Navegar al formulario de creación de paciente
        await page.goto("/paciente/crear");
        await expect(page).toHaveURL("/paciente/crear");
        ...
        const emailPromise = page.waitForRequest(
            (request) =>
        ...
        await emailPromise;
        await expect(
            page.getByText("Paciente registrado exitosamente")
        ).toBeVisible();
    ...
    test("debería mostrar error con token inválido", async ({ page }) => {
        ...
        await expect(
            page.getByText("Token de activación inválido")
        ).toBeVisible();
    });
    ...
    test("debería mostrar error con token expirado", async ({ page }) => {
        ...
        await expect(
            page.getByText("El token de activación ha expirado")
        ).toBeVisible();
    });
    ...
    test("debería activar cuenta exitosamente", async ({ page }) => {
        ...
        await expect(
            page.getByText("Cuenta activada exitosamente")
        ).toBeVisible();
    ...
    test("debería validar requisitos de contraseña", async ({ page }) => {
        ...
        const casos = [
            ...
            },
            {
                ...
                error: "La contraseña debe incluir mayúsculas",
            },
            {
                password: "ABCDEFGHIJKL",
                error: "La contraseña debe incluir minúsculas",
            },
            {
                password: "Abcdefghijkl",
                error: "La contraseña debe incluir números",
            },
            {
                password: "Abcdefgh1234",
                error: "La contraseña debe incluir caracteres especiales",
            },
        ];
    ...
    test("debería bloquear después de múltiples intentos fallidos", async ({
        page,
    }) => {
